        ...
        >
          <template v-for="item in displayColumns" :key="item.label">
            ...
            <template v-else-if="item.prop === 'short_name'">
              <el-table-column v-bind="item" show-overflow-tooltip class-name="short-name-column">
                <template #default="scope">
...
const STORAGE_KEY = 'customer_column_settings'
...
const allColumns = computed(() => {
  return props.contentConfig.propsList.map(item => {
    ...
    return {
      ...
      prop: item.prop,
      type: item.type,
      required: item.type === 'handler' // 操作列必需
    }
  })
})
...
systemStore.$onAction(({ name, after }) => {
  after(() => {
    if (name === 'newPageDataAction') {
      page.value = 1
      fetchPageListData(queryInfo, false, false) // 不显示加载动画，以便看到高亮效果
    }

    if (
      ['editDomainsIsimportAction', 'deletePageByIdAction', 'editPageDataAction', 'editCustomerRemarkAction', 'editCustomerPayDayAction'].includes(name)
    ) {
      fetchPageListData(queryInfo, false, false) // 不显示加载动画
    }
  })
})
...
const handleDeleteBtnClick = throttle(
  (row) => {
    ...
    if (row.attachment_count > 0) {
      ElMessage.warning('该客户下存在附件，请先删除附件后再删除客户')
      return
    }
