...
const queryContentSearchAsync = (queryString, cb) => {
  ...
  systemStore.postCustomerListAction({ ...allQueryInfo.value }).then(res => {
...
const handleSearchEnter = () => {
  ...
  const queryInfo = {
    ...(isGroupView ? { page: 1, pageSize: 10000 } : { page: allQueryInfo.value.page }),
    short_name: searchState.value,
    "role_name": loginStore.userInfo.role.name || '',
    "user_id": loginStore.userInfo.id
  }
