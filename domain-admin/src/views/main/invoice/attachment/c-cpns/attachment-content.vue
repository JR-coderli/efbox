...
const handleHeaderCommand = (command, column) => {
  ...
  allQueryInfo.value.page = 1
  allQueryInfo.value.pageSize = 10
  allQueryInfo.value.payment_status = command
  const condition = {...queryInfo, payment_status: allQueryInfo.value.payment_status, page: allQueryInfo.value.page, short_name: allQueryInfo.value.short_name, "role_name": loginStore.userInfo.role.name || '', "user_id": loginStore.userInfo.id} // 让页面回到第一页
